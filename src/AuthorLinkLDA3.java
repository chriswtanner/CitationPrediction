...
public class AuthorLinkLDA3 {
	...
	*/
	...
	public AuthorLinkLDA3(double alpha, int numIterations2, boolean fa, String malletInputFile, String trainingFile, String s, String metaFile) throws IOException {
	...
	public void runLinkLDA() {
		...
		for (int i=0; i<this.numIterations; i++) {
			...
			/*
			// re-normalizes each P(Z|Author) so that only 5 topics per author contribute mass towards the global
			for (int a=0; a<this.numUniqueAuthors; a++) {
				Map<Integer, Double> tmp = new HashMap<Integer, Double>();
				for (int t=0; t<this.numTopics; t++) {
					tmp.put(t, n_atWords[a][t]);
				}
				
				Iterator it = sortByValueDescending(tmp).keySet().iterator();
				int ix=0;
				List<Integer> topTopics = new ArrayList<Integer>();
				while (it.hasNext() && ix < 5) {
				...
				double total = 0;
				for (int top : topTopics) {
					total += n_atWords[a][top];
				}
				for (int t=0; t<this.numTopics; t++) {
				...
				for (int t=0; t<this.numTopics; t++) {
					tmp.put(t, n_atLinks[a][t]);
				}
				
				it = sortByValueDescending(tmp).keySet().iterator();
				ix=0;
				topTopics = new ArrayList<Integer>();
				while (it.hasNext() && ix < 5) {
					int t = (Integer)it.next();
					topTopics.add(t);
					ix++;
			...
			*/
			...
			//for (int d=0; d<numDocs; d++) {
				...
				*/
				...
				// choses the top topics
				Map<Integer, Double> tmp = new HashMap<Integer, Double>();
				for (int t=0; t<this.numTopics; t++) {
					tmp.put(t, prob_topic_given_author[a][t]);
				}
				...
				double total = 0;
				for (int top : topTopics) {
					total += prob_topic_given_author[a][top];
				}
				for (int t=0; t<this.numTopics; t++) {
					if (topTopics.contains(t)) {
						prob_topic_given_author[a][t] /= total;
					} else {
						prob_topic_given_author[a][t] = 0;
					}
				} // end of truncating to only have mass from the top 5 topics for each P(Z|Author)
			} // end of going through all authors
	...
	public void printTopics(int authorNum) {
		// prints P(t|d)
	...
	public void saveLinkLDA(String lObject) throws IOException {
		...
		AuthorLinkLDA3ModelObject tmp = new AuthorLinkLDA3ModelObject(linkToID, IDToLink, topicToLinkProbabilities, topicToWordProbabilities, authorToTopicProbabilities);
